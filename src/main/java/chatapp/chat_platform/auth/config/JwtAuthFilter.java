        ...
        if (authHeader != null && authHeader.startsWith(AuthConstants.BEARER_PREFIX)) {
            ...
            try {
                if (jwtUtil.validateToken(token)) {
                    ...
                    request.setAttribute(AuthConstants.USERNAME_ATTRIBUTE, jwtUtil.extractUsername(token));
                    request.setAttribute(AuthConstants.EMAIL_ATTRIBUTE, jwtUtil.extractEmail(token));
                    request.setAttribute(AuthConstants.ROLE_ATTRIBUTE, jwtUtil.extractRole(token));
                    logger.debug("✅ Token validated for user: {}", jwtUtil.extractUsername(token));
                } else {
